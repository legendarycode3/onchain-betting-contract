...
* ****`stakeBetToken()`****: Allows either participant to deposit the agreed wager amount into the betting contract. It capability include: Restrictint staking to only registered authorized participants, requires the exact bet amount to be sent, prevents double staking, prevents double staking, records the participant's stake.
