...
* ****`s_playerHasStaked`****: Tracks whether a participant has already submitted their wager or bet. Each participant's address maps to a boolean value indicating whether they have completed the staking process. The mapping prevents duplicate deposits from the
