...
* ****`i_betAmount`****: Represents the exact amount of ETH that each participant must deposit to enter the wager or bet. This value is established during deployment and remains constant throughout the contract's lifetime. Every participant must stake precisely this amount when calling the
